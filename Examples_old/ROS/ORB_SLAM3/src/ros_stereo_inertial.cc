...
std::atomic<bool> g_sync_thread_exited(false);
...
int main(int argc, char **argv) {
  ...
  if (argc < 4 || argc > 6) {
    cerr << endl
         << "Usage: rosrun ORB_SLAM3 Stereo_Inertial path_to_vocabulary "
            "path_to_settings do_rectify [do_equalize] [do_viewer]"
         << endl;
    ros::shutdown();
    return 1;
  }
  ...
  if (argc == 6) {
    string sbUseViewer(argv[5]);
    if (sbUseViewer == "false")
      bUseViewer = false;
  }
  ...
  ros::Subscriber sub_imu =
      n.subscribe("/camera/imu", 1000, &ImuGrabber::GrabImu, &imugb);
  ros::Subscriber sub_img_left = n.subscribe(
      "/camera/infra1/image_rect_raw", 100, &ImageGrabber::GrabImageLeft, &igb);
  ros::Subscriber sub_img_right =
      n.subscribe("/camera/infra2/image_rect_raw", 100,
  ...
  // Wait for sync_thread to confirm it has exited (with timeout)
  int wait_count = 0;
  const int max_wait = 200; // 200 * 10ms = 2 seconds max
  while (!g_sync_thread_exited.load() && wait_count < max_wait) {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    wait_count++;
  }

  if (g_sync_thread_exited.load()) {
    cout << "Sync thread exited successfully." << endl;
  } else {
    cout << "Warning: Sync thread did not exit within timeout." << endl;
  }

  // Now safe to detach and shutdown
  if (sync_thread.joinable()) {
    sync_thread.detach();
  }
  cout << "Proceeding with SLAM shutdown..." << endl;
...
void ImageGrabber::SyncWithImu() {
  ...
  while (!g_shutdown_requested.load()) {
    ...
    // Check shutdown before calling TrackStereo to avoid accessing SLAM during
    // shutdown
  ...
  cout << "SyncWithImu thread exiting..." << endl;
  g_sync_thread_exited.store(true);
}
