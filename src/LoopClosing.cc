...
void LoopClosing::InsertSparsifiedKeyFrame(std::shared_ptr<KeyFrame> pKF) {
  ...
  while (1) {
    ...
    if (!mlpSparsifiedKeyFrameQueue.empty()) {
      ...
      while (!mlpSparsifiedKeyFrameQueue.empty()) {
        std::shared_ptr<KeyFrame> pKFi = mlpSparsifiedKeyFrameQueue.front();
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
    std::shared_ptr<KeyFrame> pCurrentKF, std::shared_ptr<KeyFrame> pMatchedKF, g2o::Sim3 &gScw,
    int &nNumProjMatches, std::vector<std::shared_ptr<MapPoint>> &vpMPs,
    std::vector<std::shared_ptr<MapPoint>> &vpMatchedMPs) {
  set<std::shared_ptr<MapPoint>> spAlreadyMatchedMPs;
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    if (numOptMatches > nProjOptMatches) {
      ...
      vector<std::shared_ptr<MapPoint>> vpMatchedMP;
...
bool LoopClosing::DetectCommonRegionsFromBoW(
    std::vector<std::shared_ptr<KeyFrame>> &vpBowCand, std::shared_ptr<KeyFrame> &pMatchedKF2,
    std::shared_ptr<KeyFrame> &pLastCurrentKF, g2o::Sim3 &g2oScw, int &nNumCoincidences,
  ...
  set<std::shared_ptr<KeyFrame>> spConnectedKeyFrames = mpCurrentKF->GetConnectedKeyFrames();
  ...
  for (std::shared_ptr<KeyFrame> pKFi : vpBowCand) {
    ...
    std::vector<std::shared_ptr<MapPoint>> vpMatchedPoints =
        std::vector<std::shared_ptr<MapPoint>>(mpCurrentKF->GetMapPointMatches().size(),
                                nullptr);
    std::vector<std::shared_ptr<KeyFrame>> vpKeyFrameMatchedMP =
        std::vector<std::shared_ptr<KeyFrame>>(mpCurrentKF->GetMapPointMatches().size(),
                                nullptr);
    ...
    if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
    {
      ...
      if (bConverge) {
        ...
        set<std::shared_ptr<KeyFrame>> spCheckKFs(vpCovKFi.begin(), vpCovKFi.end());

        // std::cout << "There are " << vpCovKFi.size() <<" near KFs" <<
        // std::endl;

        set<std::shared_ptr<MapPoint>> spMapPoints;
          ...
          if (numOptMatches >= nSim3Inliers) {
            ...
            if (numProjOptMatches >= nProjOptMatches) {
              ...
              while (nNumKFs < 3 && j < vpCurrentCovKFs.size()) {
                ...
                vector<std::shared_ptr<MapPoint>> vpMatchedMPs_j;
...
bool LoopClosing::DetectCommonRegionsFromLastKF(
    ...
    int &nNumProjMatches, std::vector<std::shared_ptr<MapPoint>> &vpMPs,
    std::vector<std::shared_ptr<MapPoint>> &vpMatchedMPs) {
  set<std::shared_ptr<MapPoint>> spAlreadyMatchedMPs(vpMatchedMPs.begin(), vpMatchedMPs.end());
  nNumProjMatches = FindMatchesByProjection(
      pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
...
int LoopClosing::FindMatchesByProjection(
    std::shared_ptr<KeyFrame> pCurrentKF, std::shared_ptr<KeyFrame> pMatchedKFw, g2o::Sim3 &g2oScw,
  ...
  set<std::shared_ptr<KeyFrame>> spCurrentCovisbles = pCurrentKF->GetConnectedKeyFrames();
...
void LoopClosing::CorrectLoop() {
...

  {
    ...
    for (vector<std::shared_ptr<KeyFrame>>::iterator vit = mvpCurrentConnectedKFs.begin(),
  ...
  for (vector<std::shared_ptr<KeyFrame>>::iterator vit = mvpCurrentConnectedKFs.begin(),
       ...
       vit != vend; vit++) {
    std::shared_ptr<KeyFrame> pKFi = *vit;
    vector<std::shared_ptr<KeyFrame>> vpPreviousNeighbors =
        pKFi->GetVectorCovisibleKeyFrames();

    // Update connections. Detect new links.
    pKFi->UpdateConnections();
    LoopConnections[pKFi] = pKFi->GetConnectedKeyFrames();
    for (vector<std::shared_ptr<KeyFrame>>::iterator vit_prev = vpPreviousNeighbors.begin(),
                                      vend_prev = vpPreviousNeighbors.end();
         vit_prev != vend_prev; vit_prev++) {
      LoopConnections[pKFi].erase(*vit_prev);
    }
    for (vector<std::shared_ptr<KeyFrame>>::iterator vit2 = mvpCurrentConnectedKFs.begin(),
...
void LoopClosing::MergeLocal() {
  ...
  set<std::shared_ptr<MapPoint>> spLocalWindowMPs;
      ...
      pMergeMap->IsInertial()) // TODO Check the correct initialization
  {
    ...
    while (pKFi && nInserted < numTemporalKFs) {
      ...
      set<std::shared_ptr<MapPoint>> spMPi = pKFi->GetMapPoints();
  ...
  for (std::shared_ptr<KeyFrame> pKFi : spLocalWindowKFs) {
    ...
    set<std::shared_ptr<MapPoint>> spMPs = pKFi->GetMapPoints();
  ...
  set<std::shared_ptr<MapPoint>> spMapPointMerge;
  for (std::shared_ptr<KeyFrame> pKFi : spMergeConnectedKFs) {
    set<std::shared_ptr<MapPoint>> vpMPs = pKFi->GetMapPoints();
    spMapPointMerge.insert(vpMPs.begin(), vpMPs.end());
  }

  vector<std::shared_ptr<MapPoint>> vpCheckFuseMapPoint;
  ...
  set<std::shared_ptr<MapPoint>>::iterator itMP = spLocalWindowMPs.begin();
  ...
  vector<std::shared_ptr<KeyFrame>> vpCurrentMapKFs = pCurrentMap->GetAllKeyFrames();
  vector<std::shared_ptr<MapPoint>> vpCurrentMapMPs = pCurrentMap->GetAllMapPoints();
...
void LoopClosing::MergeLocal2() {
  ...
  // cout << "updating current map" << endl;
  {
    ...
    vector<std::shared_ptr<KeyFrame>> vpMergeMapKFs = pMergeMap->GetAllKeyFrames();
    vector<std::shared_ptr<MapPoint>> vpMergeMapMPs = pMergeMap->GetAllMapPoints();
  ...
  vector<std::shared_ptr<MapPoint>>
  ...
  vector<std::shared_ptr<KeyFrame>> aux = mpMergeMatchedKF->GetVectorCovisibleKeyFrames();
  ...
  set<std::shared_ptr<MapPoint>> spMapPointMerge;
  for (std::shared_ptr<KeyFrame> pKFi : mvpMergeConnectedKFs) {
    set<std::shared_ptr<MapPoint>> vpMPs = pKFi->GetMapPoints();
    spMapPointMerge.insert(vpMPs.begin(), vpMPs.end());
...
void LoopClosing::CheckObservations(set<std::shared_ptr<KeyFrame>> &spKFsMap1,
  ...
  cout << "----------------------" << endl;
  for (std::shared_ptr<KeyFrame> pKFi1 : spKFsMap1) {
    map<std::shared_ptr<KeyFrame>, int> mMatchedMP;
    set<std::shared_ptr<MapPoint>> spMPs = pKFi1->GetMapPoints();

    for (std::shared_ptr<MapPoint> pMPij : spMPs) {
      if (!pMPij || pMPij->isBad()) {
        continue;
      }

      map<std::shared_ptr<KeyFrame>, tuple<int, int>> mMPijObs = pMPij->GetObservations();
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap,
  ...
  for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(),
    ...
    vector<std::shared_ptr<MapPoint>> vpReplacePoints(vpMapPoints.size(),
                                       nullptr);
...
void LoopClosing::SearchAndFuse(const vector<std::shared_ptr<KeyFrame>> &vConectedKFs,
                                vector<std::shared_ptr<MapPoint>> &vpMapPoints) {
  ORBmatcher matcher(0.8);
...
void LoopClosing::ResetIfRequested() {
  ...
  if (mbResetRequested) {
    ...
    for (list<std::shared_ptr<KeyFrame>>::const_iterator it = mlpLoopKeyFrameQueue.begin();
...
void LoopClosing::RunGlobalBundleAdjustment(Map *pActiveMap,
  ...
  // tree
  {
    ...
    if (!mbStopGBA) {
      ...
      list<std::shared_ptr<KeyFrame>> lpKFtoCheck(pActiveMap->mvpKeyFrameOrigins.begin(),
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        for (set<std::shared_ptr<KeyFrame>>::const_iterator sit = sChilds.begin();
             sit != sChilds.end(); sit++) {
      ...
      //  Correct MapPoints
      const vector<std::shared_ptr<MapPoint>> vpMPs = pActiveMap->GetAllMapPoints();
