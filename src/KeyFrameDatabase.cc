...
void KeyFrameDatabase::add(std::shared_ptr<KeyFrame> pKF) {
  ...
  const DBoW3::BowVector bowVec = pKF->GetBowVector();

  for (DBoW3::BowVector::const_iterator vit = bowVec.begin(),
                                        vend = bowVec.end();
       vit != vend; vit++)
...
void KeyFrameDatabase::erase(std::shared_ptr<KeyFrame> pKF) {
  ...
  for (DBoW3::BowVector::const_iterator vit = bowVec.begin(),
    ...
    for (list<std::shared_ptr<KeyFrame>>::iterator lit = lKFs.begin(), lend = lKFs.end();
         lit != lend; lit++) {
      if (pKF == *lit) {
        lKFs.erase(lit);
        break;
...
void KeyFrameDatabase::clearMap(Map *pMap) {
  ...
  // Erase elements in the Inverse File for the entry
  for (std::vector<list<std::shared_ptr<KeyFrame>>>::iterator vit = mvInvertedFile.begin(),
                                               vend = mvInvertedFile.end();
       vit != vend; vit++) {
    // List of keyframes that share the word
    list<std::shared_ptr<KeyFrame>> &lKFs = *vit;
...
vector<std::shared_ptr<KeyFrame>> KeyFrameDatabase::DetectLoopCandidates(std::shared_ptr<KeyFrame> pKF,
                                                          float minScore) {
  ...
  for (list<std::shared_ptr<KeyFrame>>::iterator lit = lKFsSharingWords.begin(),
    ...
    if (pKFi->mnLoopWords > minCommonWords) {
      ...
      float si = mpVoc->score(bowVec, pKFi->GetBowVector());
  ...
  for (list<pair<float, std::shared_ptr<KeyFrame>>>::iterator it = lScoreAndMatch.begin(),
  ...
  for (list<pair<float, std::shared_ptr<KeyFrame>>>::iterator it = lAccScoreAndMatch.begin(),
...
void KeyFrameDatabase::DetectCandidates(std::shared_ptr<KeyFrame> pKF, float minScore,
  ...
  if (!lKFsSharingWordsLoop.empty()) {
    ...
    // Only compare against those keyframes that share enough words
    int maxCommonWords = 0;
    for (list<std::shared_ptr<KeyFrame>>::iterator lit = lKFsSharingWordsLoop.begin(),
  ...
  if (!lKFsSharingWordsMerge.empty()) {
    ...
    // Only compare against those keyframes that share enough words
    int maxCommonWords = 0;
    for (list<std::shared_ptr<KeyFrame>>::iterator lit = lKFsSharingWordsMerge.begin(),
...
void KeyFrameDatabase::DetectBestCandidates(std::shared_ptr<KeyFrame> pKF, vector<std::shared_ptr<KeyFrame>> &vpLoopCand,
...
void KeyFrameDatabase::DetectNBestCandidates(std::shared_ptr<KeyFrame> pKF, vector<std::shared_ptr<KeyFrame>> &vpLoopCand,
  ...
  // Search all keyframes that share a word with current frame
  {
    ...
    for (DBoW3::BowVector::const_iterator vit = bowVec.begin(),
      ...
      for (list<std::shared_ptr<KeyFrame>>::iterator lit = lKFs.begin(), lend = lKFs.end();
           lit != lend; lit++) {
        std::shared_ptr<KeyFrame> pKFi = *lit;
        if (!pKFi) continue;  // 防御：跳过已污染的 null 条目（Phase0 安全网）
  ...
  set<std::shared_ptr<KeyFrame>> spAlreadyAddedKF;
  int i = 0;
  list<pair<float, std::shared_ptr<KeyFrame>>>::iterator it = lAccScoreAndMatch.begin();
...
vector<std::shared_ptr<KeyFrame>> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F,
                                                                    Map *pMap) {
  list<std::shared_ptr<KeyFrame>> lKFsSharingWords;
