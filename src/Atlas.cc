...
void Atlas::PreSave() {
  cout << "Atlas::PreSave: Start" << endl;

  if (mpCurrentMap) {
    cout << "Atlas::PreSave: mpCurrentMap exists, id=" << mpCurrentMap->GetId()
         << endl;
    if (!mspMaps.empty() && mnLastInitKFidMap < mpCurrentMap->GetMaxKFid())
      mnLastInitKFidMap = mpCurrentMap->GetMaxKFid() +
                          1; // The init KF is the next of current maximum
  } else {
    cout << "Atlas::PreSave: mpCurrentMap is NULL" << endl;
  }
  ...
  cout << "Atlas::PreSave: mspMaps.size()=" << mspMaps.size() << endl;
  mvpBackupMaps.clear();
  mvpBackupMaps.reserve(mspMaps.size());
  std::copy(mspMaps.begin(), mspMaps.end(), std::back_inserter(mvpBackupMaps));
  sort(mvpBackupMaps.begin(), mvpBackupMaps.end(), compFunctor());

  std::set<GeometricCamera *> spCams(mvpCameras.begin(), mvpCameras.end());
  cout << "Atlas::PreSave: Processing " << mvpBackupMaps.size() << " maps"
       << endl;

  int mapIdx = 0;
  for (Map *pMi : mvpBackupMaps) {
    cout << "Atlas::PreSave: Map " << mapIdx << flush;
    if (!pMi) {
      cout << " is NULL, skipping" << endl;
      mapIdx++;
      continue;
    }
    if (pMi->IsBad()) {
      cout << " is Bad, skipping" << endl;
      mapIdx++;
      continue;
    }

    size_t numKFs = pMi->GetAllKeyFrames().size();
    cout << " has " << numKFs << " KFs" << flush;

    if (numKFs == 0) {
      // Empty map, erase before of save it.
      cout << " - marking as bad" << endl;
      SetMapBad(pMi);
      mapIdx++;
      continue;
    }

    cout << " - calling PreSave..." << flush;
    pMi->PreSave(spCams);
    cout << " done" << endl;
    mapIdx++;
  }
  cout << "Atlas::PreSave: RemoveBadMaps" << endl;
  RemoveBadMaps();
  cout << "Atlas::PreSave: Complete" << endl;
}
