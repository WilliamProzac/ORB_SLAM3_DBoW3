  ...
  if (mspKeyFrames.size() > 0) {
    if (pKF->mnId == mpKFlowerID->mnId) {
      vector<std::shared_ptr<KeyFrame>> vpKFs =
          vector<std::shared_ptr<KeyFrame>>(mspKeyFrames.begin(), mspKeyFrames.end());
...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s,
  ...
  for (set<std::shared_ptr<MapPoint>>::iterator sit = mspMapPoints.begin();
...
void Map::PreSave(std::set<GeometricCamera *> &spCams) {
  // Note: NOT using mutex here as nested calls would cause deadlock

  cout << "Map::PreSave: Start, MPs=" << mspMapPoints.size()
       << ", KFs=" << mspKeyFrames.size() << endl;
  ...
  for (std::shared_ptr<MapPoint> pMPi : mspMapPoints) {
    ...
    for (map<std::shared_ptr<KeyFrame>, std::tuple<int, int>>::iterator it = mpObs.begin(),
                                                         end = mpObs.end();
         it != end; ++it) {
  ...
  cout << "Map::PreSave: Processed " << mpIdx << " MapPoints" << endl;
  ...
  cout << "Map::PreSave: Saved " << mvBackupKeyFrameOriginsId.size()
       << " KF origins" << endl;
  ...
  cout << "Map::PreSave: Backed up " << savedMPs << " MapPoints" << endl;
  ...
  cout << "Map::PreSave: Backed up " << savedKFs << " KeyFrames" << endl;
  ...
  cout << "Map::PreSave: Complete" << endl;
}
...
void Map::PostLoad(
  ...
  map<long unsigned int, std::shared_ptr<MapPoint>> mpMapPointId;
  for (std::shared_ptr<MapPoint> pMPi : mspMapPoints) {
    if (!pMPi || pMPi->isBad())
      continue;
